...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if(!member.roles.cache.has("1064243104896520354")) return interaction.reply({ content: 'BU KOMUTU KULLANMAK İÇİN YETKİN YOK', ephemeral: true })
    if(interaction.channel.id !== "1067491415417954304") return interaction.reply({ content: 'Bu Komutu Burada Kullanamazsın', ephemeral: true })
        const { guild } = member;
        try {
           ...
           // interaction.reply({ content: "Yükleniyor...", ephemeral: true });
            ...
            //LOG SİSTEMİ
            client.channels.cache.get('1066672280345772070').send(`${interaction.user.tag} tarafından ${isim} ${soyisim} kişisi sorgulandı.`)
